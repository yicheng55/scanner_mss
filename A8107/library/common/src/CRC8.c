 /**************************************************************************//**
 * @file        CRC8.c
 * @version     V1.00.00
 * $Revision:   0 $
 * $Date:       2017-02-14 $
 * @brief       CRC8 calcuate
 *
 * @note
 *
 * Copyright (C) 2017 AMICCOM Electronics Corp. All rights reserved.
 *****************************************************************************/
 
#include "AMICCOM_CM0.h"

//---------------------------------------------------------
// This half lookup table is for
//  poly = x8 x7 x3 x2 1 = 1000 1101 = 0x8D
//---------------------------------------------------------
const uint8_t CRC8_8D_TABLE[256] = {
    , 
};

/**
 *
 * @brief   Calcuate the CRC8.
 *
 * @param   crc8_init           CRC8 initial value.
 * @param   buf                 Data buffer
 * @param   length              Data length
 * @return  CRC8
 */
uint8_t CRC8_Calc(uint8_t crc8_init, uint8_t *buf, uint32_t length)
{
    while(length)
    {
        crc8_init = CRC8_8D_TABLE[crc8_init ^ (*buf)];
        buf ++;
        length --;
    }
    
    return crc8_init;
}
