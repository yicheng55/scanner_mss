...
void RedDM(void)
{
	...
	spi3w_wcommand(0x11);
	spi3w_wdata(0x03);	
	SetWinAddr(0x00, 12, 0x00, 211, 0x00, 0x00);
//	SetWinAddr(0x00, 0X0F, 0x00, 0x127, 0, 0x00);
...
void Print_Char(uint8_t Col, uint8_t Line, char ascii)
{
	...
	for(j=0; j<2; j++) {
		SetWinAddr((Line*2)-1,(Line-1)*2, 211, 211-((Col+1)*8), (Line*2)-2+j, 211-(Col*8));
		for(i=0; i<8; i++) {
...
void Print_Char_BIG_Font(uint8_t Col, uint8_t Line, int Width ,int Addr,char ascii) //FOR BIG NO COMPRESS
{
	...
	for(j=0; j<3; j++) {
		...
		SetWinAddr(Line,Line+2, 211, 211-((2)*Width), (Line+j), 211 - Addr);
//		printf("%d ",Addr);
...
{
	ENTRY_MESSAGE(FLAG_MF_APPLICATION, _T("(+)EPaperInit_1675A_213\r\n"));
...
#if 1	
	spi3w_wcommand(0x01);			// 
	spi3w_wdata(0xD3);				// Set MUX as 212
	spi3w_wdata(0x00);
...
#if 0		
	spi3w_wcommand(0x04);
	spi3w_wdata(0x2D);				// VSH/VSL = 15V //2D 41 
	spi3w_wdata(0xB2);                         //B2 A8
	spi3w_wdata(0x22);                         //22 32
...
//	ENTRY_MESSAGE(FLAG_MF_APPLICATION, _T("(+)019\r\n"));
		WriteLUT();
//	Demo_Mode();
//	Write_EXAMPLE_FULL(EPAPER_COLOR_BLACK_WHITE,EPD_DATA_290_INCH);
//	UpdateDisplay();
//	Write_EXAMPLE_FULL(EPAPER_COLOR_RED,EPD_DATA_290_INCH_RED);
//	UpdateDisplay();
	
//	spi3w_wcommand(0x2C);   // Write VCOM Register
//	spi3w_wdata(0x00);//A8 //3C

//Write_EPD_FULL(EPAPER_COLOR_BLACK_WHITE,2756,black_2_13_inch);
////	UpdateDisplay();
//Write_EPD_FULL(EPAPER_COLOR_RED,2756,red_2_13_inch);
////////////	UpdateDisplay();
//printTime();
//UpdateDisplay();
//printTime();	
...
//	LinePrint_Big_Font(3,"DMT2131-");
	LinePrint_Big_Font(3,"%s",szTagMsg4);
	LinePrint_Big_Font(4,"%s",szTagID);
	UpdateDisplay();	
	LinePrint_Big_Font(1,"%s",szTagStatus);
	LinePrint_Big_Font(2,"%s_%s_%d",szTagMsg1,szTagMsg2, GetTagWakeUpInterval());
//	LinePrint_Big_Font(3,"DMT2131-");
