...
//////			printf("1. Temperature = %d   ,AdcBase=%d   ,ZeroOffsetTemperature=%.1f  \r\n", Temperature ,AdcBase, ZeroOffsetTemperature );	
////	
////		if(ConvertTemperature <= 0x00)
////		{
////			ConvertTemperature = (abs(ConvertTemperature)) | 0x80;
//////			printf("2. ValueTemperature = %d   ,TempBase=%d   ,ConvertTemperature=%.1f  \r\n", ValueTemperature ,TempBase, ConvertTemperature );
////		}
////	
////		if(GetAdcBase() == 1376)
////		{
...
////		}
//////		printf("1. ValueTemperature = %d   ,TempBase=%d   ,ConvertTemperature=%.1f  \r\n", ValueTemperature ,TempBase, ConvertTemperature );
//////		ConvertTemperature = (abs(ConvertTemperature)) | 0x80;
//////		printf("2. ValueTemperature = %d   ,TempBase=%d   ,ConvertTemperature=%.1f  \r\n", ValueTemperature ,TempBase, ConvertTemperature );
////		return (int8_t)ConvertTemperature;
////}
...
{
	int WaitingTimer = 60*1000;
	WATCHDOG->LOCK = 0x1ACCE551;
//	WATCHDOG->LOAD = 0x9FFFF60;//0xFFFFF0;
	WATCHDOG->LOAD = (((SystemCoreClock/1000)*WaitingTimer)-1);
	WATCHDOG->CTRL = 0x03;
	WATCHDOG->LOCK = 0x00;
...
{
				WATCHDOG->LOCK = 0x1ACCE551;
				WATCHDOG->INTCLR = 0x00;
//				WATCHDOG->LOAD = (((SystemCoreClock/1000)* 80 * 1000) - 1);
				WATCHDOG->LOCK = 0x00;
