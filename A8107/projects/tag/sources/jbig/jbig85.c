		       ...
		       unsigned char *prevline, unsigned char *prevprevline)
{
  ...
  int tmax;
#ifdef DEBUG
//#warning  This code will not work without the Keil Compiler.
  ...
  printf( "encode line %lu (%2lu of stripe)\n", s->y, s->i);
#endif
...
#ifdef DEBUG
  if (s->y == s->y0)
    printf( "tp_lines = %ld, encoded_pixels = %ld\n",
...
#ifdef DEBUG
	printf( "PSCD was longer than expected, unread bytes "
		"%02x %02x %02x %02x ...\n", data[*cnt], data[*cnt+1],
