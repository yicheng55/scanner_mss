    ...
    switch (pBigHead->PacketType)
    {
        ...
        case RF_PT_OVER_THE_AIR:
            ...
            else
            {
                ...
                *(P_MemCpyNode)&(*mRunTimePara).KeepRxPktStrc.RawDataBuf[mRunTimePara->KeepRxDataCnt][RxBigDataCnt] = *(P_MemCpyNode)pBigData->toTagData;
...
uint8_t Rpt_RecvTagList(P_RPT_RUNTIME_PARAMETER mRunTimePara, uint8_t *TagListBuf)
{
    ...
    switch (pTagUpdateList->PacketType)
    {
        case RF_PT_TAG_UPDATE_LIST:
            ...
            {
                pTagUpdateList = (P_TAG_UPDATE_LIST)mRunTimePara->KeepRxPktStrc.DeviceListPkt;
...
uint8_t Rpt_RecvBeacon(P_RPT_RUNTIME_PARAMETER mRunTimePara, uint8_t *SyncBuf)
{
    uint8_t mResult = 0;
    P_SYNC_BEACON pSyncBeacon   = (P_SYNC_BEACON)SyncBuf;

    //printf("%d ", (RPT_TASK_TIMER_VAL - TASK_CUR_RUNTIME(TIMER1)));
    RstTimerValue(TIMER1, RPT_TASK_TIMER_VAL + 1);
...
uint8_t Rpt_HandleRecvPktType(P_RPT_RUNTIME_PARAMETER mRunTimePara, uint8_t *TmpBuf)
{
    ...
    */
    ...
    if (mRunTimePara->RxPktLen == SYNC_LEN)
    {
        ...
        if (DecryptData[1] == 0x01)
        {
            ...
            memcpy(TmpBuf, DecryptData, mRunTimePara->RxPktLen);
            /*for(i = 0; i < mRunTimePara->RxPktLen; i++)   {
                printf("%x ", TmpBuf[i]);
            }
            printf("\r\n");
        ...
        //*(P_MemCpyNode)&(*mRunTimePara).KeepRxPktStrc.SyncPkt = *(P_MemCpyNode)TmpBuf;
        memcpy((*mRunTimePara).KeepRxPktStrc.SyncPkt, TmpBuf, mRunTimePara->RxPktLen);
    }
    ...
    switch (pRecvPktHead->PacketType)
    {
        case RF_PT_SYNC_BEACON:
            mResult = Rpt_RecvBeacon(mRunTimePara, TmpBuf);
            break;
        ...
        case RF_PT_ALWAYS_WAKE_BEACON:
            ...
            //memcpy((*mRunTimePara).KeepRxPktStrc.DeviceListPkt, TmpBuf, mRunTimePara->RxPktLen);
...
uint8_t Rpt_RxMode_RecvUpdData(P_RPT_RUNTIME_PARAMETER mRunTimePara, uint8_t *RxTmpBuf)
{
    ...
    DBGMSG("Rpt rx %02X:%02X ", pRptUpdHead->TagID[3], pRptUpdHead->TagID[4]);
