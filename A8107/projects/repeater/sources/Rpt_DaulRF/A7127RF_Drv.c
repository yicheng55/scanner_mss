...
{
		0x0D,	//Amicomm sugestion for Victor new board. //0x34, //page0, 2E
		0x49, //page1,
		0x00, //Page2,
		0x80, //page3,
		0x80, //page4,
...
uint8_t SPI_ExchangeByte(uint8_t input)
{
		...
		SPI_WRITE_TX(SPI, (uint32_t)input);		//Read command + Register offset address
...
** Strobe Command
*********************************************************************/
void A7127_StrobeCmd(uint8_t cmd)
{
		__disable_irq();
...
void A7127_SetFIFOLen(uint32_t fifolen)
{
		//uint8_t mlen[2];
...
void A7127_Cal(void)
{
		...
		A7127_StrobeCmd(CMD_PLL); //calibration @PLL state
		...
		A7127_StrobeCmd(CMD_STBY); //return to STBY state
...
void A7127RF_Enable(void)
{
		...
		A7127_StrobeCmd(CMD_STBY);
		
		A7127RF_SetCHAN(RptDefSet.Tchn);
	 
		printf("Enable A7127 RF...\r\n");
}
